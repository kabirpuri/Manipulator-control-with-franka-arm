...
class JacobianDemo():
    ...
    def ellipse(t,f=0.5,ry=.15,rz=.10):
        ...
        xdes = x0 + np.array([ry * sin(f*t), rz * cos(f * t), 0])
        vdes = np.array([ry * f * cos(f * t), -rz * f * sin(f * t), 0])

        Rdes = np.diag([1., -0.5, -0.5])
        ang_vdes = 0.0 * np.array([1.0, 0.5, 0.0])
        ## END STUDENT CODE
